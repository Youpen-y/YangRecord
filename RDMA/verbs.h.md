...
    IBV_WC_GRH      = 1 << 0,      /* GRH 存在，仅对 UD QP 有效 */ 

    IBV_WC_WITH_IMM     = 1 << 1,  /* 立即数据值 有效 */
...
struct ibv_wc {
    ...
    uint64_t        wr_id;       /* 完成的 Work Request 的 ID */

    enum ibv_wc_status  status;  /* 操作的状态 */

    enum ibv_wc_opcode  opcode;  /* 在完成的 WR 中指定的 Operation type */

    uint32_t        vendor_err;  /* 供应商错误综合 */
...
    uint32_t        imm_data;    /* 立即数据 （按网络字节顺序） */

    uint32_t        qp_num;      /* 已完成的 WR 的本地 QP number */

    uint32_t        src_qp;      /* 已完成的 WR 的源（远端） QP number（仅对 UD QPs 有效） */  

    int         wc_flags;        /* 已完成的 WR 的 标志 */

    uint16_t        pkey_index;  /* P_Key 索引 （仅对 GSI QPs 有效） */

    uint16_t        slid;        /* 源 LID */

    uint8_t         sl;          /* Service Level */

    uint8_t         dlid_path_bits; /* DLID path bits (对多播消息不适用) */

};
/*
    并非所有 wc 属性始终有效，如果 completion status 不是 IBV_WC_SUCCESS ，则只有以下属性有效：wr_id, status, qp_num, vendor_err
*/
 ```
...
    void    (*verbs_uninit_func)(struct verbs_device *device);
...
```
// ibv_context_ops 一组函数指针，是指向 Verbs API 的操作函数
