...
ht* ht_create(void) {
    ...
    if (table == NULL) {
        fprintf(stderr, "ht_create-malloc space failed");
        return NULL;
    }
    table->length = 0;
 ...
 * See more: https://en.wikipedia.org/wiki/Fowler-Noll-Vo_hash_function
 */
...
void *ht_get(ht *table, const char *key) {
    // AND hash with capcity -1 to ensure it's within entries array.
    ...
    while (table->entries[index].key != NULL && !flag) {
...
static const char* ht_set_entry(ht_entry *entries, size_t capacity, const char *key, void *value, size_t *plength) {
    ...
    // Didn't find key, but get a NULL space
    if (plength != NULL) {
        key = strdup(key);
        if (key == NULL) {
            return NULL;
