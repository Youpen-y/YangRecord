...
### clang-format 样式选项
- 既可以使用预定义的样式（ `LLVM`, `Google`, `Chromium`, `Mozilla`, `WebKit`, `Microsoft` ）之一。使用命令行中选项`-style=`
- 也可通过配置特定的样式选项来创建自定义样式。使用 `-style=file`时，每个输入文件的 `clang-format` 将尝试查找位于文件最近的父目录中的 `.clang-format` 文件。
...
简短示例：
```
BasedOnStyle: LLVM	# use LLVM style
Indentwidth: 4			# 4 columns indentation
TabWidth: 4				# number of columns used for tab

Language: Cpp
Standard: Cpp11

PointerAlignment: Left	# Pointer and reference alignment style. {Left, Right, Middle}

ColumnLimit: 100		# use 100 columns for Cpp
```
